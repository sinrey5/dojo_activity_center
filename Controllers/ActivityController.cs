...
namespace dojo_activity_center.Controllers
{
    public class ActivityController : Controller
    {
        ...
        public IActionResult AddActivity(NewActivity newActivity)
        {
            ...
            List<Activity> CreatedActivities = DbContext.Activities.Where(u => u.UserId == ActiveUser.UserId).Include( a => a.Coordinator).ToList();
            ...
            if(ModelState.IsValid)
            {
                ...
                DbContext.Add(theActivity);
                DbContext.SaveChanges();
                Activity currentActivity = DbContext.Activities.Where(a => a.UserId == ActiveUser.UserId).Last();
        ...
        {
            Participate currentParticipate = DbContext.Participate.SingleOrDefault( p => p.UserId == ActiveUser.UserId && p.ActivityId == activity_id);
            DbContext.Participate.Remove(currentParticipate);
            DbContext.SaveChanges();
            return RedirectToAction("Index");
        }
        [HttpGet]
        ...
        {
            Activity theActivity = DbContext.Activities.SingleOrDefault(a => a.ActivityId == activity_id);
        ...
        {
            Activity theActivity = DbContext.Activities.Include(a => a.Participants).ThenInclude(p => p.ParticipatingUsers).SingleOrDefault(a => a.ActivityId == activity_id);
        ...
        public DashboardModels InitializeDashboard()
        {
            ...
            {
                AllActivities = DbContext.Activities.OrderBy(u => u.Date).Include( a => a.Participants).ThenInclude(p => p.ParticipatingUsers).ToList(),
                AllUsers = DbContext.Users.Include( u => u.CreatedActivities).ToList(),
                User = ActiveUser,
